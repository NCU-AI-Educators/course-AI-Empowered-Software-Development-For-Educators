...
### 顶点课程 (Capstone Project)
这是工程教育中的经典概念，源自建筑学中放置在拱门顶部的最后一块石头。
它不仅是对过往知识的综合应用（Python + Web + AI），更是连接“学习者”与“创造者”身份的关键桥梁。
在之前的课程中，我们是在制造 **“砖块”**（写函数、处理数据）；而今天，我们要把这些砖块砌成一座完整的 **“大厦”**（可交付的 Web 应用）。
...
软件生命周期的常态不是“从零开始”，而是“持续演进”。

我们今天践行的是**重构 (Refactoring)** 与 **增量开发 (Incremental Development)** 的理念：
在不抛弃旧有逻辑（核心算法）的基础上，通过添加新的一层（Web 接口），让旧代码焕发新生。
...
### 为什么选择局域网 (LAN)部署？
相比云端 SaaS (如问卷星)，本地部署有四大优势：
1.  **防作弊**: 物理层面的反作弊（必须连接教室 WiFi）。
2.  **低延迟**: 抢答毫秒级响应，体验更刺激。
3.  **强隐私**: 学生的成绩数据不上传公有云，符合合规要求。
4.  **零成本**: 不需要买服务器，老师的笔记本就是服务器。
...
### 数据清洗 (Data Cleaning / ETL)
在数据科学中，这叫 **ETL** (Extract, Transform, Load)。
- **E**: 从 Word/PDF 中提取文字。
- **T**: 转换为 Markdown 格式。
- **L**: 保存为 `.md` 文件供 AI 读取。
这是数据工程中最基础也最重要的环节。
...
### 静态图表 vs 动态图表
- **静态 (Static)**: 像报纸上的图，信息量固定，用户是被动的。
- **动态 (Interactive)**: 像股票软件，用户可以与数据对话（筛选、缩放）。
Web 的核心优势就在于这种**探索式的数据体验**。
...
### 提示词工程 (Prompt Engineering) 的三板斧
1.  **拆解 (Decompose)**: 把大问题化小。
2.  **生成 (Generate)**: 让 AI 写样板代码 (Boilerplate)。
3.  **修复 (Debug)**: 利用 AI 的上下文理解能力进行错误诊断。
掌握这三招，你就能驾驭 80% 的未知技术领域。
...
### 代码评审 (Code Review)
这是专业开发团队的标配环节。
**旁观者清**。你自己写的代码，自己往往会有思维盲区。
引入他人的视角，不仅能发现 Bug，更是知识共享（Knowledge Sharing）的最佳途径。
...
### 课程大拼图：构建数字生命
我们可以把开发软件比作创造一个**生命体**：
- **前5章 (Python)**: 打造了**骨骼与肌肉**。赋予了程序**行动**的能力（处理数据、操作文件）。
- **第6章 (Web)**: 长出了**皮肤与五官**。赋予了程序**感知**的能力（漂亮的界面、与用户对话的接口）。
- **第7章 (AI)**: 将植入**大脑**。赋予程序**思考**的能力（理解自然语言、逻辑推理）。
这三步走完，我们就从简单的“写代码”，进化到了**“用 AI 创造 AI”**的**造物主模式**。

...
# =============================================================================
# 《AI赋能软件开发》课程
# 模块三，第11课 起始代码 (v1.2 - 史诗级更新)
#
# 本文件是第10课“结对调试”练习之后，已经修复了Bug的版本。
# 关键的改动在`handle_go`函数中，我们增加了“防御性”代码：
# 在尝试移动到新地点之前，会先检查该地点是否存在于`world`字典中。
# 这使得即使数据有误（如此处我们故意留下的、指向不存在的'market'的出口），
# 程序也不会崩溃，而是能优雅地提示错误。
#
# v1.2更新:
...
# 游戏世界 (包含一个指向不存在地点的“Bug”)
world = {
    'guangchang': {
        'name': '扬州广场',
        'description': '你现在位于扬州城的中心广场，人来人往，热闹非凡。四周的建筑鳞次栉比。\n往南是一家客栈，往北是一家茶馆，往西是铁匠铺，往东是药铺。',
        # 这里我们故意留下一个错误数据，为第10课的调试练习做准备
        'exits': {'south': 'kezhan', 'north': 'chaguan', 'west': 'tjiangpu', 'east': 'market'},
        'items': ['布告栏']
    },
    'kezhan': {
        'name': '客栈',
...
def handle_go(argument, world_data, current_room_data, current_location):
    """处理移动指令，返回新的玩家位置。包含防御性代码。"""
    if not argument:
        print("请指定一个方向，例如 '/go east'。")
        return current_location

    direction = argument
    if direction in current_room_data['exits']:
        destination_id = current_room_data['exits'][direction]
        
        # --- 防御性代码 ---
        if destination_id in world_data:
            return destination_id
        else:
            print(f"错误：地图数据损坏，地点 '{destination_id}' 未定义。")
            return current_location
        # --- 防御性代码结束 ---
...
def handle_ask(argument, npcs_data, player_location):
    ...
    try:
        ...
        topic = parts[1].strip()
        if not target_npc_name or not topic:
            raise IndexError
    except IndexError:
        print("指令格式不完整。请使用: /ask [NPC名字] about [话题]")
        return
    ...
    for npc_id, npc_data in npcs_data.items():
        if npc_data['location'] == player_location and npc_data['name'].lower() == target_npc_name:
            ...
            response = npc_data['dialogue'].get(topic, f"{npc_data['name']}想了想，说：“这个……我也不太清楚。”")
...
def process_command(command_input, player_location, inventory):
    ...
    if command == '/quit':
        ...
        new_location = handle_go(argument, world, current_room_data, player_location)
